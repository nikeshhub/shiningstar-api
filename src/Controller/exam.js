...
import { generateExamNoticePDF, generateStudentExamNotice, ensureUploadsDir } from "../utils/pdfGenerator.js";
    ...
    for (const [familyId, group] of familyGroups) {
      ...
      for (const student of group.students) {
        try {
          ...
          const filename = `exam_notice_${exam._id}_${student.studentId}_${Date.now()}.pdf`;
          const outputPath = path.join(uploadsDir, filename);

          // Generate PDF
          await generateStudentExamNotice({
            exam,
            student,
          ...
          // Store PDF URL in student document (optional - we can add this field to schema if needed)
          console.log(`Generated exam notice for student ${student.studentId}: ${filename}`);
        } catch (error) {
...
export let enterMarks = async (req, res) => {
  try {
    const { studentId, examId, classId, academicYear, terminalNumber, subjectMarks, enteredBy } = req.body;
...
export let bulkEnterMarks = async (req, res) => {
  try {
    const { examId, classId, academicYear, terminalNumber, studentsMarks, enteredBy } = req.body;
...
export let generateExamNotice = async (req, res) => {
  try {
    ...
    // Generate PDF filename
    const filename = `exam_notice_${exam._id}_${Date.now()}.pdf`;
    const outputPath = path.join(uploadsDir, filename);

    // Generate PDF
    await generateExamNoticePDF(exam, outputPath);

    // Update exam with PDF URL
    exam.noticeGenerated = true;
    exam.noticeGeneratedAt = new Date();
    exam.noticePdfUrl = `/uploads/pdfs/${filename}`;
    await exam.save();
...
export let downloadExamNotice = async (req, res) => {
  try {
    ...
    const filePath = path.join(process.cwd(), exam.noticePdfUrl);

    res.download(filePath, `exam_notice_${exam.examName.replace(/\s/g, '_')}.pdf`, (err) => {
      if (err) {

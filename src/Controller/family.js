  ...
  try {
    ...
    if (studentCount > 0) {
      return res.status(400).json({
        success: false,
        message: `Cannot delete family. ${studentCount} student(s) are linked to this family. Please unlink them first.`,
      });
    }
...
export let unlinkStudentFromFamily = async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message: "Student unlinked from family successfully",
      data: student
    });
  } catch (error) {
    handleError(res, error);

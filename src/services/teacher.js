...
export let deleteTeacher = async (req, res) => {
  try {
    ...
    if (timetableCount > 0 || attendanceCount > 0) {
      return res.status(409).json({
        success: false,
        message: `Cannot delete teacher with linked records: ${timetableCount} timetable slot(s), ${attendanceCount} attendance record(s).`
      });
    }
